const fedwire_participants = [];

const get = async (aba) => {
  const url = `https://www.frbservices.org/EPaymentsDirectory/fedwireResults.html?bank=&aba=${aba}&state=+&city=+&_securities=on&_securities=on&_funds=on&_funds=on&submitButton=Search&referredBy=searchFedwirePage`;
...
const init = async () => {
  ...
  console.log(fedwire_participants);
};
